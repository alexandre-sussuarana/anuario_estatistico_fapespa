# Você está no script de download das tabelas do Anuário estatístico 2020 do
# tado do Pará. Para acessar o painel de informações, execute o comando abaixo.
browseURL('https://www.fapespa.pa.gov.br/sistemas/anuario2020/')

# Observações -----------------------------------------------------------------

# Na seção Infraestrutura, a tabela 8 não está disponível nem em sua versão
# html.
...
link = 'https://www.fapespa.pa.gov.br/sistemas/anuario2020/demografia.html'
...
link = 'https://www.fapespa.pa.gov.br/sistemas/anuario2020/economia.html'
...
link <-'https://www.fapespa.pa.gov.br/sistemas/anuario2020/infraestrutura.html'
...
for (i in 1:length(t0)) {         # LOOP DE OBTENCAO DAS TABELAS POR TABELA
  ...
  if (class(d) == "try-error") {
    print(paste0('tabela: ', i, ';'))
    print(paste('erro:', d))
    infra$nome[i] = d[1]
    infra$tabela[i] = gsub(retirar, t0[i], link)
    next
  }
...
link <- 'https://www.fapespa.pa.gov.br/sistemas/anuario2020/meioambiente.html'
...
for (i in 1:length(t0)) {         # LOOP DE OBTENCAO DAS TABELAS POR TABELA
  ...
  if (class(d) == "try-error") {
    ...
    print(paste0(rep('▲▼', 25), collapse = ''))
    meio$nome[i]   = d[1]
    meio$tabela[i] = gsub(retirar, t0[i], link)
    next
  }
...
link = 'https://www.fapespa.pa.gov.br/sistemas/anuario2020/social.html'
...
saveRDS(
  object = anuario20,
  file = '01_base_de_dados/anuario2020.RDS'
)
